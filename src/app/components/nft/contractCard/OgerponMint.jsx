...
const OgerponMint = ({}) => {
    ...
    function mintEasy(contract) {
        console.log(account)
        contract.methods.requestRandomNumber(oracle, jobId).send( { from: account, gasPrice: "250000000000", gas: 210000, value: 1000000000000000000 } ).then(response => {
            console.log(response)
            alert('交易成功,隨機值的獲取可能會需要一點時間,回到首頁查看新的NFT!')
        }).catch(err => {
            console.log(err)
        })
    }
